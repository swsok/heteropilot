...
#### S7.4 V3-R 본체 (카드 필요, 반나절) — **rev 4: S7.3이 선정을 무효화했고 판정 규칙을 고정한다**

> **개정 rev 4 (2026-09-21).** S7.3이 개루프 도메인을 실측한 결과 **S7.0의 후보
> 선정이 무효**가 됐다. 아래 첫 줄("S7.0이 고른 P2-형·P3-형 각 1개")은 그대로
> 실행할 수 없다. 무엇을 대신 측정하고 어떻게 판정하는지를 **측정 전에** 여기서
> 고정한다(사용자 결정, 2026-09-21).
>
> **S7.3이 바꾼 것**: 개루프 도메인은 6점, sim L 11.730~37.965, `tpot_p99` 기준
> (`profiles/calibration/openloop/rngd_card.accuracy.openloop.yaml`). 오차가
> +2.02 %에서 −17.63 %로 **부호를 바꾸고**, 페어링 상한이 **sim L 37.965**로
> 실측 확정됐다(2.25 rps에서 −27.85 %로 가드 이탈).

**(i) 두 케이스의 성격이 바뀌었고, 바뀐 채로 유지한다.**

| 케이스 | S7.0의 의도 | **S7.4에서 측정할 것** |
| --- | --- | --- |
| **P2** (sim L 74.498) | 발명이 잡아내는 후보 | **참양성**: `sim feasible / planner refuse / measured saturated`. 시뮬레이터는 통과시키고, 플래너는 검증 영역 밖이라 보류하며, 실측은 포화다. 보류가 옳았음을 보이는 사례이지 마진 크기를 재는 사례가 아니다 |
| **P3** (sim L 37.965) | 발명이 구제하는 후보 | **도메인 반전**: 폐루프 도메인 7.51 %로는 feasible → 개루프 p99 도메인 21.4 %로는 infeasible → 실측이 어느 쪽인지 판정. 구제 사례가 아니라 **어느 도메인을 조회하느냐가 판정을 뒤집는다**는 사례다 |

P2는 sim L 74.498이 페어링 상한의 약 두 배이므로 **개루프로 마진을 매길 수 없다**.
그것이 결함이 아니라 이 케이스의 내용이다 — 보류가 발화하는 지점을 보이는 것.

**(ii) 검증 영역 안 추가 후보 1~2개**를 고른다. 목적은 도메인이 실제로 쓰이는
구간(sim L 11.73~37.97)에서 판정이 맞는지 보는 것이다.

**(iii) 판정 규칙 — 분리폭이 산포의 3배 미만이면 우열을 판정하지 않는다.**

```
분리폭 >= 3 x (해당 동작점의 run-to-run p99 산포)  ->  우열 판정
분리폭 <  3 x 산포                                  ->  "구별 불가"를 판정으로 기록
```

"구별 불가"는 판정 실패가 아니라 **판정 결과**다. 측정이 분해하지 못한다는 사실
자체가 결과이고, 그렇게 적는다. 통과/미통과로 반올림하지 않는다.

**산포는 추정하지 않고 S7.3의 실측값을 쓰거나 그 동작점에서 새로 잰다.** S7.3이
남긴 값: **0.116 ms (0.27 %) @ sim L 37.965**, **0.776 ms (1.97 %) @ sim L
32.475** — 산포는 **동작점마다 다르고 낮은 부하에서 더 크다**. 한 값을 전 구간에
쓰지 않는다.

참고로 S7.0의 P3 분리폭 0.236 ms는 L 37.965의 산포 0.116 ms의 **2.0배**이므로,
이 규칙 아래에서는 **"구별 불가"**로 기록된다.

**(iv) 모든 판정 옆에 산포를 병기한다.** 예외 없다. 표의 모든 판정 셀에 그
동작점의 산포와 반복 횟수가 따라붙어야 하고, 단일 런이면 "산포 미상"이라고 쓴다.

**(v) flake 처리**: 실패한 테스트는 **1회만** 재실행하고, 재실행했다는 사실과
결과를 **PR에 기록**한다. `retry`·`xfail` 표시는 금지. **빨간 스위트는 머지하지
않는다**.

- ~~S7.0이 고른 **P2-형·P3-형 각 1개**를 개루프로 3회씩 돌린다.~~ (rev 4: 위 (i)로 대체)
- 예측은 `plan`이
...
- **실행 간 p99 산포를 판정 옆에 적는다** — 위 (iii)·(iv)가 이 항목을 정량화한
  것이다. V3에서 P3이 0.623 ms로 통과한 것을 보고할 때 요구된 규율이 출발점이고,
  rev 4는 "산포보다 좁으면 잡음"이라는 정성 기준을 **3배 문턱**으로 바꾼다.
  S7.3 이전에 여기 적혀 있던 "RNGD의 폭 2.3~7.6 ms"는 **폐루프 도메인에서 도출한
  값이라 더는 유효하지 않다** — 개루프 도메인의 마진이 다르고, 실측 산포도
  이제 추정이 아니라 측정값이 있다.
