...
*`WORK_ORDER_domain_scoping.md` STEP S2, from the `D110–D119` block that work
order claims in `CLAUDE.md`. It amends how absolute rule A1 of
`WORK_ORDER_uncertainty_planner.md` is enforced, so read that rule and D33
first. STEP S1 lands on its own branch and takes the block's first number.*
