...
| **GPU→host leg of the cross-vendor KV path** | ✅ **done 2026-08-27** (A40 server) | 26.03 GB/s single stream, 79.98 GB/s across 8 GPUs, pinned; the GPU leg saturates ~80 GB/s and reaches only 38 % of ideal at 8 streams against the NPU leg's 88 %. Composed serialised, the cross-vendor fixture links are ~15 GB/s, so the old 35 GB/s placeholder was ~2.3× too optimistic — still above Exp 3's ~10 GB/s crossing. `experiments/results/gpu_host_bandwidth.md` |
