...
envelope measurement of D22 (`experiments/results/rngd_concurrency_envelope.md`)
...
The measurement in D22 shows performance is a **function of the operating
...
# value. Conflating the two is what produced the D22 retraction: a 24-request
...
- **`concurrency_metric: served`** is mandatory and the loader rejects an
  envelope without it. The requested/served conflation is exactly what D22
  retracts.
- **`validity` is mandatory, and `refuse` is the default.** Silent extrapolation
  past the measured range is the root cause of the whole D22 episode.
...
The most transferable lesson from D22. Extend `profiles/calibration/<hw>.yaml`:
...
Mixing them silently is the same class of error as D22.
