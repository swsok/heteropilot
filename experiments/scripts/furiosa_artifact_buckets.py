...
def emit_yaml(path: Path, doc: dict, rows: list[dict], out) -> None:
    ...
    print("# Generated by experiments/scripts/furiosa_artifact_buckets.py --yaml -- do not hand-edit.", file=out)
    print("#", file=out)
    ...
    print("# model's input; it does not change the hardware. See WORK_ORDER_npu_exec_model_spike.md A7.", file=out)
    ...
    for want in ("prefill", "extend", "decode"):
        ...
        for row in group:
            print(
                ...
                f" kv_cache_size: {row['kv_cache_size']}, input_ids_size: {row['input_ids_size']}}}",
                file=out,
            )
...
def main() -> int:
    ...
    prefill_max = max((r["attention_size"] for r in rows if r["kind"] in ("prefill", "extend")), default=0)
