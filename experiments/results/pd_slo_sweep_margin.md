...
original 10 rps with `--tpot-margin-percent 18`, the TPOT optimism D21 measured at
the concurrency these plans run at. Artifacts:
`outputs/pd_slo_sweep_margin18/{pd-rngd-gpu-card,pd-rngd-gpu}.json`.
This is the re-run D21 §4.4 asks for.*
...
| --- | --- | --- |
| arrival rate | 10 rps | 10 rps (unchanged — see D21 on why lowering it fails) |
...
card-fixture winner runs at (D21, `rngd_concurrency_envelope.md`). Applying it
